...
document.addEventListener("DOMContentLoaded", function() {
    // We'll define initDataTable() so we can call it from app.js
    window.initDataTable = function() {
      ...
      // Initialize the DataTable with a custom template that:
      // - Places the per-page dropdown on the left
      // - Places the export dropdown next to it
      // - Places the search input on the right
      const table = new simpleDatatables.DataTable(tableEl, {
        ...
        layout: {
          top: "",      // We'll define a custom top with `template`
          bottom: ""    // We'll define a custom bottom if needed
        },
        template: (options, dom) => {
          return `
            <div class="${options.classes.top}">
              <div class="flex flex-col sm:flex-row sm:items-center space-y-4 sm:space-y-0 
                          sm:space-x-3 rtl:space-x-reverse w-full sm:w-auto">
                ...
                <div id="exportDropdown"
                  class="z-10 hidden w-52 divide-y divide-gray-100 rounded-lg bg-white shadow-sm 
                         dark:bg-gray-700" data-popper-placement="bottom"
                >
              ...
              ${
                options.searchable
                  ? `<div class="${options.classes.search} relative mt-4 sm:mt-0">
                       <input 
                         class="${options.classes.input} dataTable-input 
                                pl-10 pr-4 py-2 block w-full rounded-lg border 
                                text-sm text-gray-900 focus:ring-primary-500 focus:border-primary-500" 
